...
class Cell:
    ...
    def _construct_cell(self, hoc_file):
        """
        Loads cell morphology and biophysics.
        
        For L5 and L23 neuron, loads both from .hoc file.
        For RGCs loads via class RGC which itself refers partly to hoc files.
        """
        if "RGC" in hoc_file:
            from rgcTemplate import RGC
            self.RGC = RGC(gid, ID, Stim, x=0, y=0, z=0, theta=0)
        else:
    ...
    def _move_to_cortical_position(self, cortical_depth):
        ...
        if cortical_depth == None:
            ...
            print(
                "Warning: Parameter cortical_depth not when cell was initialized. The parameter was set to %4f um."
                % self.cortical_depth
            )
        else:
            if "L23" in self.hoc_file:
    ...
    def plot_foutz2012(self, func, axes='xz', scaling = 1, segfunc=False, clim=None,cmap=None,norm=None,color=None, shift_x=0, shift_y=0, shift_z=0):
