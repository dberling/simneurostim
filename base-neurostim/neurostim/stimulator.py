...
class Stimulator:
    ...
    def calculate_Tx_at_pos(self, pos_xyz_um: list, stim_xyz_um: list) -> float:
        ...
        """
        if not (isinstance(pos_xyz_um, list) and isinstance(stim_xyz_um, list)):
            raise ValueError("pos_xyz_um and stim_xyz_um must be lists of [x, y, z] coordinates in micrometers.")
...
class MultiStimulator:
    ...
    def calculate_combined_intensity_at_pos(self, pos_xyz_um: list) -> float:
        """
        ...
        for stim in self.stimulators:
            intensity = stim.calculate_Tx_at_pos(pos_xyz_um, stim.position)
